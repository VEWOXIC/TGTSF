...
if [ ! -d "./logs/fixTGTSF+PatchTST_Steam_pretrain-mpnet" ]; then
    mkdir ./logs/fixTGTSF+PatchTST_Steam_pretrain-mpnet

fi
seq_len=60
model_name=TGTSF_PatchTST
...
model_id_name=TGTSF_steam100-mpnet_keymask_cross5_self3
data_name=investing
...
do
    ...
    do
        ...
        --patience 10\
        --target 'Players' \
        --itr 1 --batch_size $bs --learning_rate 5e-5 #| tee logs/fixTGTSF+PatchTST_Steam_pretrain-mpnet/keymask_lr5e-4-$model_name'_'$model_id_name'_'$seq_len'_'$pred_len'_bs'$bs'-100-5cross-3self'.log 
    done
done
