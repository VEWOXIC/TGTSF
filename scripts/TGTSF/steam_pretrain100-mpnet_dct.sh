...
if [ ! -d "./logs/TGTSF_Steam_pretrain-mpnet-dct" ]; then
    mkdir ./logs/TGTSF_Steam_pretrain-mpnet-dct

fi
seq_len=60
model_name=TGTSF_dct
...
for gameid in 107410 #for debug
...
do
    ...
    do
        ...
        --patience 10\
        --target 'Players' \
        --itr 1 --batch_size $bs --learning_rate 1e-4   | tee logs/TGTSF_Steam_pretrain-mpnet-dct/lr1e-4-$model_name'_'$model_id_name'_'$seq_len'_'$pred_len'_bs'$bs'-100'.log 
    done
done
